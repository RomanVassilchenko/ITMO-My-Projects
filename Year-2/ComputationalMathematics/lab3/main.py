...
# Функции для проверки на сходимость и разрывы
def check_convergence(func, a, b):
    if func == f4 and ((a >= -math.inf and b <= 0) or (a >= 0 and b <= math.inf)):
        ...
        return True
    elif func == f1 or func == f2 or func == f3:
        return True
    else:
        return False
...
# Изменение функции вычисления интеграла для обработки несобственных интегралов
...
def main():
    ...
    for i, method in enumerate(methods, 1):
        print(f"{i}. {method}")
    choice = int(input("Ваш выбор: "))
    method = list(methods.keys())[choice - 1]
